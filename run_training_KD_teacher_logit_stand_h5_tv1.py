...
from dataset.dcase24_ntu_teacher_ensem_logit_stand_tv1 import ntu_get_training_set_dir, ntu_get_test_set, ntu_get_eval_set, open_h5, close_h5
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if self.logit_stand:
            ...
            with torch.cuda.amp.autocast():
                ...
                teacher_logits = F.log_softmax(teacher_logits / self.config.temperature, dim=-1)
        kd_loss = self.kl_div_loss(y_hat_soft, teacher_logits).mean() # do we need to sum(1)?
...
def train(config):
    ...
    ################################################## Don't forget to close the h5 files at the end of training ##########################################################
    ############ h5 edit end #################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="NTU_Var1-T_32BCBL-S_FMS_DIR_sub5_fixh5")
