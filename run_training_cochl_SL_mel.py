...
from dataset.cochl_ntu_teacher_10s import ntu_get_training_set_dir, ntu_get_test_set, ntu_get_eval_set, open_h5, close_h5, ntu_get_test_set_cochl
...
class PLModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW([
            ...
            {'params': head_params, 'lr': 0.05},             # High learning rate for classifier heads
...
def train(config):
    ...
    wandb_logger = WandbLogger(
        ...
        notes="BCBL pre-trained on CochlScene",
        tags=["NTU24"],
    ...
    val_dl = DataLoader(dataset=ntu_get_test_set(hf_in),
                         worker_init_fn=worker_init_fn,
                         num_workers=config.num_workers,
                         batch_size=config.batch_size,
                         pin_memory=True,
                         )
    test_dl = DataLoader(dataset=ntu_get_test_set_cochl(hf_in),
                         worker_init_fn=worker_init_fn,
                         num_workers=config.num_workers,
                         batch_size=config.batch_size,
                         pin_memory=True,
...
def evaluate(config):
    ...
    eval_hf = open_h5('h5py_cochl_test_mel_10s')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="NTU_BCBL_cochl_SL_FG_5e-3_Cf_5e-2_h5")
